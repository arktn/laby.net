...
async function test_server() {
    ...
    console.log(`User vote by UUID:\n${await laby.checkVote('gommehd.net', '34e57efa-5783-46c7-a9fc-890296aaba1f')}`);
    console.log(`User vote by UUID error:\n${await laby.checkVote('gommehd.net', '61699b2e-d327-4a01-9f1e-0ea8c3f06bc6')}`);
    console.log(`User vote by nickname:\n${await laby.checkVote('gommehd.net', 'Niklas')}`);
    console.log(`User vote by nickname error:\n${await laby.checkVote('gommehd.net', 'Dinnerbone')}`);
    console.log('\n');
}

function credits() {
    console.log('Data:\nTest-Users: Niklas & Dinnerbone\nTest-Server: gommehd.net');
}
test_uuid();
